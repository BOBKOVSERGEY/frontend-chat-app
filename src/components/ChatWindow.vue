...
const setupListeners = (roomId: number | string) => {
  echo
    ...
    .listen('MessageSentEvent', async (e: { message: Message }) => {
      await useRoomStore().addMessage(e.message, chatRoomId.value);
      await scrollToBottom();
    });

  echo
    .private(`chatroom.${roomId}`)
    .listen('UserIsTypingEvent', (e) => {
      console.log('user is typing', e);
      if (usersTyping.value.indexOf(e.username) === -1)
        usersTyping.value.push(e.username);
    })
    .listen('UserStoppedTypingEvent', (e: { username: string; roomId: number }) => {
